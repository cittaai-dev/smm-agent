...
export function BridgePairCard({ pair, claim }: { pair: BridgePair; claim?: VerifiedClaim }) {
  return (
    <div className="rounded-lg border border-border p-4" data-testid="bridge-pair-card">
      <p className="text-xs font-medium uppercase tracking-wide text-text-faint">Observed (brand data)</p>
      <p className="mb-3 mt-1 text-sm text-text">{pair.run_chunk.text}</p>
      <p className="text-xs font-medium uppercase tracking-wide text-text-faint">
        Market Intel Core benchmark
      </p>
      <p className="mt-1 text-sm text-text">{pair.core_chunk.text}</p>
      {claim && (
